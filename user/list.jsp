...
<%
    ...
    try
    { 
        ord = Integer.parseInt(request.getParameter("order")); 
    }
    catch(Exception e){}
    ...
    switch(ord)
    {
    case 1: case -1: order = User.ORDER_LOGIN; break;
    case 2: case -2: order = User.ORDER_DATE; break;
    case 3: case -3: order = User.ORDER_AMOUNT; break;
    }
    if(ord < 0)
        order += " desc";
    
    java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("dd.MM.yyyy");
    String href = "/page.jsp?id=" + pageId;
    int num = User.getUserCount();
    Enumeration enum = User.getUserList(order, pageNumber, pageSize).elements();
%>
...
<tr valign="top" <%=l%2==1?"bgcolor=\"#F1F1F1\"":""%>>
	...
	<td class="smallFont"><a href="/pages/<%=langCode%>/forum/profile?login=<%=CMSApplication.toHTML(uuu.getLogin())%>"><b><%=CMSApplication.toHTML(uuu.getLogin())%></b></a></td>
