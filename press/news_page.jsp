...
if(pressIds != null || previewInfo != null)
{
	...
	{
		Vector v = new Vector();
		enum = Press.getPress(pressIds, new News());
...
<tr>
	<td nowrap><img src="<%=des%>/blu-ttl-lft.gif" width="16" height="19" border="0"><img src="<%=des%>/company/ttl-press-center.gif" height="19" alt="Пресс-центр" border="0"></td>
...
<tr valign="top">
	...
	<td></td>
	<td></td>
	<td width="100%"><a href="<%=nodePath(pageId)%>"><img src="<%=des%>/company/btn-nws-lst.gif" height="18" alt="Вернуться к списку новостей" border="0"></a><br></td>
</tr>
<!----//5 ----->
</table>
<%
...
else
{
	...
	int pageSize = News.getPagePressAmount();
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("dd.MM.yyyy");
	int num = Press.getPressNum(startDate, finishDate, new News(DBObject.LOAD_EXTENDED_LIST), pageId, true);
	Enumeration enum;
	if(num > 0 || inArc)
		enum = Press.getPress(startDate, finishDate, pageNumber, pageSize, new News(DBObject.LOAD_EXTENDED_LIST), pageId, true);
	else
	{
		enum = Press.getPress(3, new News(DBObject.LOAD_EXTENDED_LIST), pageId, true);
		num = 3;
	}
%>
