...
if(pressIds != null || previewInfo != null)
{
	...
	{
		Vector v = new Vector();
		enum = Press.getPress(pressIds, new Article());
...
<%
	while(enum.hasMoreElements())
	{
		Article news = (Article)enum.nextElement();
%>
<!---- 1 ----->
<tr>
...
<I class="c66"><%= CMSApplication.toHTML(news.getPublisher())%></I>
<div style="margin-top:8px;">
<%=news.getContent()%>
</div>
</td>
...
<tr valign="top">
	...
	<td></td>
	<td></td>
	<td width="100%"><a href="<%=nodePath(pageId)%>"><img src="<%=des%>/press/btn-prs-lst.gif" height="18" alt="Вернуться к списку публикаций" border="0"></a><br></td>
</tr>
<!----//5 ----->
</table>
<%
...
else
{
	...
	int pageSize = Article.getPagePressAmount();
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("dd.MM.yyyy");
	...
	int num = Press.getPressNum(startDate, finishDate, new Article(DBObject.LOAD_EXTENDED_LIST), pageId, true);
	Enumeration enum;
	if(num > 0 || inArc)
	{
		enum = Press.getPress(startDate, finishDate, pageNumber, pageSize, new Article(DBObject.LOAD_EXTENDED_LIST), pageId, true);
	}
	else
	{
		enum = Press.getPress(3, new Article(DBObject.LOAD_EXTENDED_LIST), pageId, true);
		num = 3;
	}
%>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
...
<tr>
<form action="<%=nodePath(pageId)%>" method="get" name="arc_form">
...
<tr valign="top">
	...
	<div style="margin-top:3px;" class="defFont"><I class="c66"><%= news.getPublisher()%></SPIAN></div>
	</td>
</tr>
<!----//1 ----->
