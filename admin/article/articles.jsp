...
%>
<%
	Enumeration enum = CMSApplication.getApplication().getNodesByTypeCode(langCode, Article.SERVICE_CODE);
...
<jsp:param name="header" value="Ленты публикаций"/>
...
<%=sPrivate%><A HREF="article_list.jsp?page_id=<%=node.getId()%>"><%=buildPath(node)%></A>
<%		}else{ %>
...
<NOBR><%		if(currentAdmin.isW(node.getId())) { %>
<A HREF="article_edit.jsp?page_id=<%=node.getId()%>&id=">добавить статью</A>
<%		}else{ %>
добавить
<%		} %></NOBR>
</TD>
...
<TABLE BORDER="0" CELLSPACING="1" CELLPADDING="1" WIDTH="700">
<FORM METHOD="post" ACTION="xt_articles.jsp">
<INPUT TYPE="hidden" NAME="mode" VALUE="set_num">
<TR>
<TD HEIGHT="20" CLASS="c1"><DIV STYLE="padding-left:6px;padding-right:6px">
Количество статей на странице:&nbsp;&nbsp;&nbsp;
<INPUT TYPE="text" NAME="press_num" SIZE="5" VALUE="<%=Article.getPagePressAmount()%>"><BR>
</DIV></TD>
</TR>
<TR>
